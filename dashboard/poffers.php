    ...
    <div class="row" id="cont">
        <div class="col-md-12 col-sm-12 col-xs-12">
            <div class="x_panel">
                <div class="x_title">
                    <h2> العروض المُعلقة
                        <small>عروض بانتظار الموافقة</small> 
                    </h2><br/>
                ...
                <div class="x_content">
                    <table id="datatable-fixed-header" class="table table-striped table-bordered">
                        ...
                        if(USER_RANK ==1 )
                        {
                            $q = "SELECT * FROM `offers` WHERE `status` = 0 ORDER BY `date` DESC ";
                        }
                        else
                        {
                            $uid = USER_ID;
                            $q = "SELECT * FROM `offers` WHERE (`status` = 0 AND `uid` = '$uid') ORDER BY `date` DESC ";
                        }
                        ...
                        <tr id="products<?=$show['pid']?>">
                            ...
                            <th>
                                <form name="ratio">
                                    <input type="text" value="<?=$showu['ratio']?>" placeholder="نسبة مئوية  (10,15...)" class="form-control" name="ratio">
                                </form>
                            </th>
                            <th>
                                <button class="btn btn-danger btn-flat" onclick="del('offers','<?=$show[pid]?>')"><i class="fa fa-trash"></i></button>
                                <a href="eedit?edit=<?=$show['pid']?>" class="btn btn-success btn-flat"><i class="fa fa-edit"></i></a>
                                <button class="btn btn-primary btn-flat" onclick="acc_pro('<?=$show[pid]?>','offers')"><i class="fa fa-check"></i></button>
                            </th>
                        </tr>
                        <?}?>
                        </tbody>
